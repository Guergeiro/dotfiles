...
if exists('$SUDO_USER')
  ...
  set backup
  set writebackup
  &backupdir = expand('/tmp/.vimtrash/backup//')
  if !isdirectory(expand(&backupdir))
    mkdir(expand(&backupdir), 'p')
  endif
  set swapfile
  &directory = expand('/tmp/.vimtrash/swapfiles//')
  if !isdirectory(expand(&directory))
    mkdir(expand(&directory), 'p')
  endif
  if has('persistent_undo')
    set undofile
    &undodir = expand('/tmp/.vimtrash/undodir//')
...
enddef
defcompile
nnoremap <silent> K <ScriptCmd><sid>ShowDocumentation()<cr>
# AutoCommands {{{ #
augroup Autocmd
  autocmd!
...
if empty(glob(expand('$HOME') .. '/.vim/autoload/plug.vim'))
  if has('dialog_con')
    var choice = confirm('Install VimPlug?', '&Yes\n&No', 2)
    if choice == 1
